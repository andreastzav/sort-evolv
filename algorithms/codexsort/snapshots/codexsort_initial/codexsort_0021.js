...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  while (left < middle && right < end) {
    if (compare(array[left], array[right]) <= 0) {
      scratch[write - scratchOffset] = array[left];
      left += 1;
    } else {
      scratch[write - scratchOffset] = array[right];
      right += 1;
    }
    write += 1;
  }
  ...
  for (let i = start; i < end; i += 1) {
    array[i] = scratch[i - scratchOffset];
  }
}
...
function codexRunMergeSort(array, compare) {
  ...
  const scratch = new Array((array.length >>> 1) + 1);
  const stack = [];
