...
const MIN_RUN_LARGE = 40;
const MIN_RUN_MID_BAND = 36;
const MIN_RUN_HUGE = 64;

const SMALL_RANGE_LIMIT = 48;
const MERGE_BLOCK_SIZE = 32;
const FULL_SPAN_MERGE_MIN_LENGTH = 32768;
const FULL_SPAN_MERGE_MAX_LENGTH = 65536;
...
const LOW_CARDINALITY_BUCKET_MAX_CLASSES = 96;
...
function chooseMinRun(length) {
  ...
  if (length >= FULL_SPAN_MERGE_MIN_LENGTH && length < FULL_SPAN_MERGE_MAX_LENGTH) {
    return MIN_RUN_MID_BAND;
  }
  if (length < 65536) {
...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  if (
    array.length >= FULL_SPAN_MERGE_MIN_LENGTH &&
    array.length < FULL_SPAN_MERGE_MAX_LENGTH
  ) {
    mergeStableFullSpan(array, scratch, start, middle, end, compare);
    return;
  }
...
function mergeStableFullSpan(array, scratch, start, middle, end, compare) {
  let left = start;
  let right = middle;
  let write = start;
