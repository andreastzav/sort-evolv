...
const MIN_RUN_LARGE = 45;
const MIN_RUN_LARGE_UPPER = 44;
const MIN_RUN_HUGE_LOWER = 54;
const MIN_RUN_HUGE = 54;

const SMALL_RANGE_LIMIT = 48;
const MERGE_BLOCK_SIZE = 32;
const QUICK_MAX_DEPTH_MULTIPLIER = 2;
const MERGE_TRIM_MEDIUM_SKIP_UPPER = 8192;
const MERGE_TRIM_MIN_SPAN = 704;

const LOW_CARDINALITY_SAMPLE_COUNT = 25;
const LOW_CARDINALITY_CLASS_RATIO = 0.54;
const LOW_CARDINALITY_MIN_LENGTH = 2304;
const LOW_CARDINALITY_BUCKET_MAX_CLASSES = 54;
...
function chooseMinRun(length) {
  ...
  if (length < 8192) {
    return MIN_RUN_LARGE_UPPER;
  }
  if (length < 15872) {
    return MIN_RUN_HUGE_LOWER;
  }
  return MIN_RUN_HUGE;
}
...
function upperBound(array, start, end, pivot, compare) {
  let left = start;
  let right = end;
  while (left < right) {
    const middle = (left + right) >>> 1;
...
function lowerBound(array, start, end, pivot, compare) {
  let left = start;
  let right = end;
  while (left < right) {
    const middle = (left + right) >>> 1;
...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  const mergeTrimMinSpan = array.length < 20000 ? 1536 : array.length < 100000 ? 704 : MERGE_TRIM_MIN_SPAN;
  if (array.length >= MERGE_TRIM_MEDIUM_SKIP_UPPER && end - start >= mergeTrimMinSpan) {
    end = lowerBound(array, middle, end, array[middle - 1], compare);
    if (middle >= end) {
      return;
    }
  }
  ...
  if (rightLength * 2 < leftLength) {
    for (let i = 0; i < rightLength; i += 1) {
      scratch[i] = array[middle + i];
    }

    let left = middle - 1;
    let right = rightLength - 1;
    let write = end - 1;

    while (left >= start && right >= 0) {
      if (compare(array[left], scratch[right]) > 0) {
        array[write] = array[left];
        left -= 1;
      } else {
        array[write] = scratch[right];
        right -= 1;
      }
      write -= 1;
    }
  ...
  while (left < leftLength && right < end) {
    if (compare(scratch[left], array[right]) <= 0) {
      array[write] = scratch[left];
      left += 1;
    } else {
      array[write] = array[right];
      right += 1;
    }
    write += 1;
  }
...
function collapseRunStack(array, scratch, runs, compare) {
  while (runs.length > 1) {
    ...
    if (
      array.length >= 246272
        ? runLength(runs[n]) < runLength(runs[n + 1])
        : runLength(runs[n]) <= runLength(runs[n + 1])
    ) {
      mergeRunAt(array, scratch, runs, n, compare);
      continue;
...
function hasLowCardinalitySignal(array, compare) {
  ...
  const sampleLimit = length < 20000 ? 24 : LOW_CARDINALITY_SAMPLE_COUNT;
  const sampleCount = Math.min(sampleLimit, length);
  if (sampleCount < 8) {
    return false;
  }

  const step = Math.max(1, Math.floor(length / sampleCount));
  const classRatio = length < 100000 ? 0.56 : LOW_CARDINALITY_CLASS_RATIO;
  const classes = [];
  ...
  for (let i = 0; i < sampleCount; i += 1) {
    ...
    if (!matched) {
      classes.push(value);
      if (classes.length > sampleCount * classRatio) {
        return false;
      }
    }
  }

  return classes.length <= sampleCount * classRatio;
}

function tryStableBucketSort(array, compare) {
  const reps = [];
  const buckets = [];
  const maxClasses = LOW_CARDINALITY_BUCKET_MAX_CLASSES;
