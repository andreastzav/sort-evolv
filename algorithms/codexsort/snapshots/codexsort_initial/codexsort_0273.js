...
const MERGE_TRIM_MEDIUM_SKIP_UPPER = 75000;
const QUICK_SORT_SMALL_LIMIT = 1024;

const LOW_CARDINALITY_SAMPLE_COUNT = 32;
const LOW_CARDINALITY_CLASS_RATIO = 0.57;
const LOW_CARDINALITY_MIN_LENGTH = 2048;
const LOW_CARDINALITY_QUICK_CLASS_LIMIT = 20000;
const LOW_CARDINALITY_BUCKET_MAX_CLASSES = 56;
...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  const allowTrim = array.length >= MERGE_TRIM_MEDIUM_SKIP_UPPER;
  if (allowTrim) {
...
function tryStableBucketSort(array, compare) {
  ...
  const buckets = [];
  const maxClasses =
    array.length < LOW_CARDINALITY_QUICK_CLASS_LIMIT
      ? LOW_CARDINALITY_BUCKET_MAX_CLASSES
      : LOW_CARDINALITY_BUCKET_MAX_CLASSES - 1;
...
function codexSortInPlace(array, compare) {
  ...
  if (
    ...
    (length < LOW_CARDINALITY_QUICK_CLASS_LIMIT ||
      length >= MERGE_TRIM_MEDIUM_SKIP_UPPER) &&
