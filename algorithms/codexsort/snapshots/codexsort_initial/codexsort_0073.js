...
const MIN_RUN_LARGE = 44;
const MIN_RUN_HUGE = 55;
const MIN_RUN_MASSIVE = 53;
...
const LOW_CARDINALITY_BUCKET_MAX_CLASSES = 60;
...
function tryStableBucketSort(array, compare) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    for (let classIndex = 0; classIndex < reps.length; classIndex += 1) {
      const order = compare(value, reps[classIndex]);
      if (order === 0) {
    ...
    for (let shift = reps.length; shift > insertAt; shift -= 1) {
      reps[shift] = reps[shift - 1];
      buckets[shift] = buckets[shift - 1];
    }
    reps[insertAt] = value;
    buckets[insertAt] = [value];
  }
