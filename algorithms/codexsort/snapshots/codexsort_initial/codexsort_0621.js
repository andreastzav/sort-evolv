...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  if (array.length >= MERGE_TRIM_MEDIUM_SKIP_UPPER && end - start >= MERGE_TRIM_MIN_SPAN) {
    start = upperBound(array, start, middle, array[middle], compare);
    if (start >= middle) {
      return;
    }
...
function hasLowCardinalitySignal(array, compare) {
  ...
  const classRatio = length >= 20000 && length < MERGE_TRIM_MEDIUM_SKIP_UPPER
    ? LOW_CARDINALITY_MEDIUM_CLASS_RATIO
    : LOW_CARDINALITY_CLASS_RATIO;
  const classes = [];
