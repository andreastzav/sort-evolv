...
const MIN_GALLOP = 7;
const MIN_GALLOP_SKIP = 4;
const MIN_GALLOP_ARRAY_LENGTH = 65536;
const MIN_GALLOP_MERGE_LENGTH = 8192;
...
function mergeStable(array, scratch, start, middle, end, compare) {
  ...
  let rightWins = 0;
  const allowGallop =
    array.length >= MIN_GALLOP_ARRAY_LENGTH &&
    end - start >= MIN_GALLOP_MERGE_LENGTH;
  ...
  while (left < middle && right < end) {
    if (compare(array[left], array[right]) <= 0) {
      ...
      if (allowGallop && leftWins >= MIN_GALLOP && left < middle) {
        if (
          left + MIN_GALLOP_SKIP <= middle &&
          compare(array[left + MIN_GALLOP_SKIP - 1], array[right]) <= 0
        ) {
      ...
      if (allowGallop && rightWins >= MIN_GALLOP && right < end) {
        if (
          right + MIN_GALLOP_SKIP <= end &&
          compare(array[right + MIN_GALLOP_SKIP - 1], array[left]) < 0
        ) {
