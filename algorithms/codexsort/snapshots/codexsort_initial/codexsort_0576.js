...
const MERGE_TRIM_MEDIUM_SKIP_UPPER = 100001;
const MERGE_TRIM_MIN_SPAN = 57;
...
function hasLowCardinalitySignal(array, compare) {
  ...
  const sampleLimit = length >= 65536 ? 29 : LOW_CARDINALITY_SAMPLE_COUNT;
...
function tryStableBucketSort(array, compare) {
  ...
  const buckets = [];
  const maxClasses =
    array.length >= 65536 ? LOW_CARDINALITY_BUCKET_MAX_CLASSES : 53;
