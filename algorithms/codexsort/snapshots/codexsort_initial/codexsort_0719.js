...
function collectAdaptiveRuns(array, compare) {
  ...
  while (cursor < length) {
    ...
    if (runEnd - runStart < minRun && length - runStart >= minRun) {
      let forcedEnd = Math.min(length, runStart + minRun);
      if (
        length >= 100000 &&
        length - forcedEnd > 0 &&
        length - forcedEnd < minRun * 2
      ) {
        forcedEnd = length;
      }
