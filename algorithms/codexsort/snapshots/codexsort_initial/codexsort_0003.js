...
const PARTITION_PATH_MIN_LENGTH = 4096;
const LARGE_SCALE_PARTITION_LENGTH = 50000;
...
function mergeSortRange(array, scratch, start, end, compare) {
  ...
  for (let blockStart = start; blockStart < end; blockStart += MERGE_BLOCK_SIZE) {
    ...
    bubblePreconditionRange(array, blockStart, blockEnd, compare);
...
function collectAdaptiveRuns(array, compare) {
  ...
  while (cursor < length) {
    ...
    if (runEnd - runStart < minRun) {
      ...
      bubblePreconditionRange(array, runStart, forcedEnd, compare);
...
function codexSortInPlace(array, compare) {
  ...
  if (length >= LARGE_SCALE_PARTITION_LENGTH) {
    return stablePartitionQuickSort(array, compare);
  }

  if (length >= PARTITION_PATH_MIN_LENGTH && hasLowCardinalitySignal(array, compare)) {
    return stablePartitionQuickSort(array, compare);
  }
