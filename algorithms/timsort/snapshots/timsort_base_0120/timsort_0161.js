const FastTimSort = (() => {
  const DEFAULT_MIN_MERGE = 76;
  const DEFAULT_MIN_GALLOPING = 11;
  const DEFAULT_TMP_STORAGE_LENGTH = 288;
  const BULK_COPY_THRESHOLD = 768;
  const MAX_TIMSORT_STATE_POOL_SIZE = 2;
  const MAX_POOLED_TMP_LENGTH = DEFAULT_TMP_STORAGE_LENGTH * 96;
  ...
  function copyFromBuffer(destArray, destStart, srcArray, srcStart, count) {
    ...
    if (count <= 4) {
      switch (count) {
        case 4:
          destArray[destStart + 3] = srcArray[srcStart + 3];
        // falls through
        case 3:
          destArray[destStart + 2] = srcArray[srcStart + 2];
        // falls through
        case 2:
          destArray[destStart + 1] = srcArray[srcStart + 1];
        // falls through
        case 1:
          destArray[destStart] = srcArray[srcStart];
      }
      return;
    }
