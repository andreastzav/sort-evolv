const FastTimSort = (() => {
  const DEFAULT_MIN_MERGE = 40;
  const DEFAULT_MIN_GALLOPING = 8;
  const DEFAULT_TMP_STORAGE_LENGTH = 256;
  const BULK_COPY_THRESHOLD = 512;
  const ENABLE_INDEX_SORT_PATH = false;
  const INDEX_SORT_MIN_LENGTH = 120000;
  ...
  function shouldUseIndexSortPath(array) {
    if (!ENABLE_INDEX_SORT_PATH) {
      return false;
    }

    if (!Array.isArray(array) || array.length < INDEX_SORT_MIN_LENGTH) {
      return false;
    }
  ...
  function sortByIndexProjectionInPlace(array, compare) {
    const length = array.length;
    ...
    timSortInPlace(indices, function compareIndices(leftIndex, rightIndex) {
      return compare(array[leftIndex], array[rightIndex]);
    });

    const projected = new Array(length);
    for (let i = 0; i < length; i += 1) {
      projected[i] = array[indices[i]];
    }

    copyFromBuffer(array, 0, projected, 0, length);
    return array;
  }
  ...
  function FastTimSort(array, compareFn) {
    ...
    if (shouldUseIndexSortPath(array)) {
      return sortByIndexProjectionInPlace(array, compare);
    }
